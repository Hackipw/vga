...
if(expirydate>today):
    ...
    First = now.replace(hour=14, minute=25, second=0, microsecond=0)
    Firstend = now.replace(hour=15, minute=00, second=0, microsecond=0)
    Second = now.replace(hour=17, minute=00, second=0, microsecond=0)
    ...
    Secondend = now.replace(hour=15, minute=00, second=0, microsecond=0)
    Third = now.replace(hour=17, minute=25, second=0, microsecond=0)
    Thirdend = now.replace(hour=18, minute=00, second=0, microsecond=0)
    Final = now.replace(hour=20, minute=25, second=0, microsecond=0)
    Finalend = now.replace(hour=21, minute=00, second=0, microsecond=0)
    ...
    while(True):
        print(f"{red}After You Pay to The UPI ID above Your hack Automatically")
        print(f"Activate Hack By Entering The Correct command ")
        print(f"{green}(UTR) Or Upi Reference Number") 
        print(f"{neon}To Activate The Hack")
        print(f"If It Does'nt Open Contact Me On Telegram {yellow}@RXCE_HACKING")
        print(f"{neon}*---------*----------*-------------*----------*")
        print(f"{red}*---------*----------*-------------*----------*")
        print("payhere--- @RXCE_HACKING : ")
        print("If you have already paid to  UPI")
        print(f"{neon}CONTACT TELEGRAM @RXCE_HACKING")
        ...
        if(bhai==code or bhai==test or bhai==code1 or bhai==code2):
            ...
            print("28th Jan 2022, 02:30 PM- 03:00 PM")
            print("28th Jan 2022, 05:30 PM- 06:00 PM")
            print("28th Jan 2022, 08:30 PM- 09:00 PM")
            print("Please play on the given time, and ")
            print(f"If you think it is an {red}error {yellow}contact {green}me ")
            print(f"{neon}On Telegram {red}@RXCE_HACKING")
            print("wait.... starting....")
            time.sleep(20)
            period=rava
            hero()
